...
int main(int argc, char **argv) {
  ...
  int intersect_bar;
  int topk;
  std::string outpath0,outpath,datapath_a,datapath_b;
  desc.add_options()
  ...
  int ierr = MPI_File_open(MPI_COMM_WORLD, datapath_a.c_str(), MPI_MODE_RDONLY, MPI_INFO_NULL, &in);
  if (ierr) {
    solo_log(rank,0,boost::format("Couldn't open file  %1% ...") %datapath_a);
    MPI_Finalize();
    exit(2);
  }
  solo_log(rank,0,boost::format("parallel_read start at  %1% ...") %boost::posix_time::second_clock::local_time());
  parallel_read(&in, rank, world.size(), overlap,load_data_a);
  MPI_File_close(&in);

  ierr = MPI_File_open(MPI_COMM_WORLD, datapath_b.c_str(), MPI_MODE_RDONLY, MPI_INFO_NULL, &in);
  if (ierr) {
    solo_log(rank,0,boost::format("Couldn't open file  %1% ...") %datapath_a);
    MPI_Finalize();
    exit(2);
  ...
  solo_log(rank,0,boost::format("parallel_read start at  %1% ...") %boost::posix_time::second_clock::local_time());
  parallel_read(&in, rank, world.size(), overlap,load_data_b);
  MPI_File_close(&in);
  // parallel_dump(outpath0,rank,load_data);

  // data all to all
  solo_log(rank,0,boost::format("all_to_all a start at  %1% ...") %boost::posix_time::second_clock::local_time());
  std::vector<DataSet> idata_a(size),idata_b(size);
  all_to_all(world,load_data_a,idata_a);
  load_data_a.clear();
  all_to_all(world,load_data_b,idata_b);
  load_data_b.clear();
  ...
  construct_data(idata_a,data_a);
  construct_data(idata_b,data_b);
